/**
  ...
  Denford NovaTurn post processor configuration.

  $Revision: 1 $
  $Date: 2016-12-25 16:53:45 $
  
*/

description = "Denford NovaTurn";
vendor = "Denford";
legal = "Copyright (C) 2012-2016 by Autodesk, Inc.";
certificationLevel = 0;
minimumRevision = 24000;

longDescription = "Turning post for Denford Novaturn ";
...
if (programComment) {
  ...
  //csc  writeBlock(gFeedModeModal.format(98), gPlaneModal.format(18));
    writeBlock(gFeedModeModal.format(98));
  } else {
    // csc writeBlock(gAbsIncModal.format(90), gFeedModeModal.format(95), gPlaneModal.format(18));
    writeBlock(gAbsIncModal.format(90), gFeedModeModal.format(95));
  }
...
function onSection() {
  ...
  if (insertToolCall) {
    ...
    writeBlock("M06 T" + toolFormat.format(tool.number * 100 + compensationOffset)); //csc
  ...
  if (workOffset > 0) {
    if (workOffset > 6) {
      ...
      if (p > 300) {
        ...
        //csc  writeBlock(gFormat.format(54.1), "P" + p); // G54.1P
...
var resetFeed = false;
var endThreadMove = false;

function onLinear(_x, _y, _z, feed) {
  if (isSpeedFeedSynchronizationActive()) {
	
error(localize("Martham Engineering - Speed-feed synchronization is not supported for linear moves. USE CANNED CYCLE"));	
  ...
  /*  resetFeed = true;
	...
	if(endThreadMove){
		endThreadMove = false;
		writeBlock(gFormat.format(1), xOutput.format(_x), yOutput.format(_y), zOutput.format(_z), pitchOutput.format(1/threadsPerInch));	
	}else{
		endThreadMove = true;
		writeBlock(gFormat.format(92), xOutput.format(_x), yOutput.format(_y), zOutput.format(_z), pitchOutput.format(1/threadsPerInch));
		//writeComment("Martham Engineering - Linear-Thread-Turning - Use Canned Cycle");
  }
  */
	return;
  }
  ...
  if (x || y || z) {
    if (pendingRadiusCompensation >= 0) {
      pendingRadiusCompensation = -1;
//csc      writeBlock(gPlaneModal.format(18));
      switch (radiusCompensation) {
      case RADIUS_COMPENSATION_LEFT:
      ...
      default:
		...
		writeBlock(gFormat.format(isSpeedFeedSynchronizationActive() ? 32 : 1), x, y, z, f);
...
function onCircular(clockwise, cx, cy, cz, x, y, z, feed) {
  ...
  if (isFullCircle()) {
    ...
    case PLANE_XY:
      ...
      break;
    case PLANE_ZX:
      writeBlock(conditional(properties.type != "A", gAbsIncModal.format(90)),  gMotionModal.format(clockwise ? 2 : 3), iOutput.format(cx - start.x, 0), kOutput.format(cz - start.z, 0), getFeed(feed));
      break;
    case PLANE_YZ:
    ...
    case PLANE_XY:
      ...
      break;
    case PLANE_ZX:
      writeBlock(conditional(properties.type != "A", gAbsIncModal.format(90)),  gMotionModal.format(clockwise ? 2 : 3), xOutput.format(x), yOutput.format(y), zOutput.format(z), iOutput.format(cx - start.x, 0), kOutput.format(cz - start.z, 0), getFeed(feed));
      break;
    case PLANE_YZ:
...
function onCyclePoint(x, y, z) {
	...
	switch (cycleType) {
  case "chip-breaking":
	writeComment("Lathe Drilling Cycle: chip-breaking - Untested on Novamill");
	writeBlock( gFormat.format(74), "R" + rFormat.format(cycle.chipBreakDistance));
	writeBlock(	gFormat.format(74), zOutput.format(z), "Q" + spatialFormat.format(cycle.incrementalDepth * 1000), feedOutput.format(cycle.feedrate));
	break;
  case "deep-drilling":
	writeComment("Martham Lathe Drilling Cycle: deep-drilling");
    ...
    writeBlock(
      gFormat.format(codes[properties.type]),
	...
	//Martham Engineering - Force the feed and z on to the next command
	forceFeed();
	forceXYZ();
	break;
...
/* Commented by Martham Engineering - Oct 2018	
  switch (cycleType) {
  case "thread-turning":
    var r = -cycle.incrementalX; // positive if taper goes down - delta radius
    var threadsPerInch = 1.0/cycle.pitch; // per mm for metric
	...
	//Martham Engineering - Force the feed and z on to the next command
	writeComment("Martham Engineering - Thread-Turning");
	forceFeed();
	forceXYZ();
    return;
  }
