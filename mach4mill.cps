...
function onSection() {
  ...
  if (insertToolCall ||
    ...
    if (tool.spindleRPM < 1) {
      ...
      warning(localize("Spindle speed is zero"));
      //return;
    }
    if (tool.spindleRPM > 4000) {
      warning(localize("Spindle speed exceeds maximum value."));
    }
    if (tool.spindleRPM > 500) {
    writeBlock(
      sOutput.format(tool.spindleRPM), mFormat.format(tool.clockwise ? 3 : 4)
    );
    
    
    onDwell(6) // Marthamengineering - allow spindle to speed up
    }
  }
...
function onClose() {
  ...
  if (properties.useG28) {
    //writeBlock(gFormat.format(28), gAbsIncModal.format(91), "Z" + xyzFormat.format(machineConfiguration.getRetractPlane())); // retract
    writeComment("Martham Engineering - Home Z using G53 not G28");
    writeBlock(gAbsIncModal.format(90), gFormat.format(53), gFormat.format(0), "Z0.0");
    zOutput.reset();
  }
  ...
  if (!machineConfiguration.hasHomePositionX() && !machineConfiguration.hasHomePositionY()) {
    if (properties.useG28) {
      //writeBlock(gFormat.format(28), gAbsIncModal.format(91), "X" + xyzFormat.format(0), "Y" + xyzFormat.format(0)); // return to home
        writeComment("Martham Engineering - Do Not Home XY - G28");
    }
  } else {
    var homeX;
    ...
    writeBlock(gAbsIncModal.format(90), gFormat.format(53), gMotionModal.format(0), "Z0.0");
    writeComment("Martham Engineering - Do Not Home XY - G90");
  }
